...
<template>
    <AppLayout title="Plugins">
        <div class="max-w-3xl mx-auto space-y-5">
            <h2 class="text-xl font-semibold text-ink">Plugins</h2>

            <div v-if="plugins.length" class="space-y-3">
                <div v-for="plugin in plugins" :key="plugin.slug" class="bg-surface rounded-lg shadow-sm p-5 flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            <h3 class="text-sm font-semibold text-ink">{{ plugin.name }}</h3>
                            <span class="text-xs text-ink-subtle">v{{ plugin.version }}</span>
                        ...
                        <p class="text-sm text-ink-muted mt-1">{{ plugin.description }}</p>
                        <p class="text-xs text-ink-subtle mt-0.5">Author: {{ plugin.author }}</p>

                        <!-- Hooks -->
                        <div v-if="plugin.hooks && Object.keys(plugin.hooks).length" class="mt-2">
                            <p class="text-xs text-ink-subtle mb-1">Hooks:</p>
                            <div class="flex flex-wrap gap-1">
                                <span v-for="(handler, hook) in plugin.hooks" :key="hook"
                                    class="px-2 py-0.5 bg-surface-sunken text-ink-muted text-xs rounded">
                                    {{ hook }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="shrink-0">
                        <button v-if="!isEnabled(plugin.slug)"
                            ...
                            class="px-3 py-1.5 bg-primary hover:bg-primary-dark text-white text-xs font-medium rounded-lg">
                            Enable
                        </button>
                        <button v-else
