    ...
    <form @submit.prevent="emit('submit')" class="space-y-5">
        <div class="bg-white rounded-lg shadow-sm p-6 space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea v-model="form.description" rows="2" class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm resize-none" />
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 space-y-4">
            ...
            <div v-for="(f, i) in form.features" :key="i" class="flex items-center gap-2">
                ...
                <button type="button" @click="removeFeature(i)" class="text-gray-400 hover:text-red-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <p v-if="!form.features.length" class="text-sm text-gray-400">No features added.</p>
        </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <Link href="/admin/plans" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancel</Link>
