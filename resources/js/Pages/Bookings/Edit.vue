...
const form = useForm({
    ...
    event_type: b.event?.type ?? 'wedding',
    event_date: b.event?.date ?? '',
    setup_time: b.event?.setup_time ?? '',
    event_start_time: b.event?.start_time ?? '',
    event_end_time: b.event?.end_time ?? '',
    guest_count: b.event?.guest_count ?? '',
    total_amount: b.financial?.total ?? '',
    paid_amount: b.financial?.paid ?? '',
...
function submit() {
    form.put(`/admin/bookings/${b.id}`);
}
</script>

<template>
    <AppLayout :title="`Edit — ${b.booking_number}`">
        <div class="max-w-2xl mx-auto space-y-4">
            <div class="flex items-center gap-3">
                <Link :href="`/admin/bookings/${b.id}`" class="text-ink-subtle hover:text-ink-muted">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                </Link>
                <h2 class="font-display text-xl font-semibold text-ink">Edit Booking</h2>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-surface rounded-lg shadow-sm p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        <InputLabel for="total_amount" value="Total (LKR) *" />
                    ...
                    <div>
                        <InputLabel for="paid_amount" value="Paid (LKR)" />
                ...
                <div class="flex items-center justify-between pt-2">
                    <Link :href="`/admin/bookings/${b.id}`" class="px-4 py-2 text-sm text-ink-muted border border-border rounded-lg hover:text-ink">Cancel</Link>
