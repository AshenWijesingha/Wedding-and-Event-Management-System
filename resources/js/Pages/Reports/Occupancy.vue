        ...
        <div class="space-y-5">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-semibold text-ink">Occupancy Report</h2>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div class="bg-surface rounded-lg shadow-sm p-5">
                    <h3 class="text-sm font-semibold text-ink mb-4">Monthly Occupancy Rate — {{ period }}</h3>
                    <div class="flex items-end gap-2 h-48">
                        <div v-for="m in monthlyOccupancy" :key="m.label" class="flex-1 flex flex-col items-center gap-1">
                            <span class="text-xs text-ink-subtle">{{ m.occupancy_pct > 0 ? m.occupancy_pct + '%' : '' }}</span>
                ...
                <div class="bg-surface rounded-lg shadow-sm p-5">
                    <h3 class="text-sm font-semibold text-ink mb-4">Venue Occupancy — {{ period }}</h3>
                    <div v-if="venueOccupancy.length" class="space-y-3">
                        <div v-for="v in venueOccupancy" :key="v.venue">
                            <div class="flex justify-between text-sm mb-1">
                                <span class="font-medium text-ink truncate">{{ v.venue }}</span>
                                <span class="text-ink-subtle ml-2 shrink-0">{{ v.booked_days }} days ({{ v.occupancy_pct }}%)</span>
                            </div>
                    ...
                    <p v-else class="text-sm text-ink-subtle">No venue data.</p>
                </div>
            </div>
                ...
                <table class="min-w-full text-sm divide-y divide-border">
                    <thead class="bg-surface-muted">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-ink-subtle uppercase">Bookings</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-ink-subtle uppercase">Occupancy %</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-border">
                        <tr v-for="m in monthlyOccupancy" :key="m.label" :class="m.bookings > 0 ? '' : 'text-ink-subtle'">
