            ...
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <span :class="['inline-flex mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium capitalize', statusColors[quotation.status] ?? 'bg-gray-100 text-gray-600']">
                            {{ quotation.status }}
                        </span>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <button v-if="['sent', 'viewed'].includes(quotation.status)" @click="act('reject')"
                        class="inline-flex items-center gap-1.5 bg-white border border-red-300 text-red-600 hover:bg-red-50 text-sm font-medium px-3 py-2 rounded-lg">
                        Reject
                    </button>
                    <a :href="`/admin/quotations/${quotation.id}/print`" target="_blank" rel="noopener"
                        class="inline-flex items-center gap-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 text-sm font-medium px-4 py-2 rounded-lg">
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="(item, i) in quotation.items" :key="i">
                            ...
                            <td class="px-6 py-4 text-right text-sm text-gray-600">{{ money(item.unit_price) }}</td>
                            <td class="px-6 py-4 text-right text-sm font-medium text-gray-900">{{ money(item.total) }}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="border-t border-gray-200 px-6 py-4 flex justify-end">
                    <dl class="w-full max-w-xs space-y-1 text-sm">
                        <div class="flex justify-between"><dt class="text-gray-500">Subtotal</dt><dd class="text-gray-900">{{ money(quotation.financial?.subtotal) }}</dd></div>
                        <div class="flex justify-between"><dt class="text-gray-500">Discount</dt><dd class="text-gray-900">-{{ money(quotation.financial?.discount) }}</dd></div>
                        <div class="flex justify-between"><dt class="text-gray-500">Tax</dt><dd class="text-gray-900">{{ money(quotation.financial?.tax) }}</dd></div>
                        <div class="flex justify-between border-t border-gray-200 pt-1 font-semibold"><dt class="text-gray-900">Total</dt><dd class="text-indigo-600">{{ money(quotation.financial?.total) }}</dd></div>
                    </dl>
                </div>
            </div>

            <!-- Notes / Terms -->
            <div v-if="quotation.notes || quotation.terms_and_conditions" class="bg-white rounded-lg shadow-sm p-6 space-y-4">
                <div v-if="quotation.notes">
                    <h3 class="text-sm font-semibold text-gray-900">Notes</h3>
                    <p class="text-sm text-gray-600 mt-1 whitespace-pre-line">{{ quotation.notes }}</p>
                </div>
                <div v-if="quotation.terms_and_conditions">
                    <h3 class="text-sm font-semibold text-gray-900">Terms &amp; Conditions</h3>
                    <p class="text-sm text-gray-600 mt-1 whitespace-pre-line">{{ quotation.terms_and_conditions }}</p>
                </div>
            </div>
        </div>
    </AppLayout>
