    ...
    <AppLayout title="Tasks">
        <div class="space-y-4">
            ...
            <div v-if="showCreate" class="bg-white rounded-lg shadow-sm p-5">
                <h3 class="text-sm font-semibold text-gray-900 mb-3">New Task</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <select v-model="createForm.priority" class="border-gray-300 rounded-md text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <select v-model="createForm.assigned_to" class="border-gray-300 rounded-md text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <textarea v-model="createForm.description" placeholder="Description (optional)" rows="2"
                        class="sm:col-span-2 border-gray-300 rounded-md text-sm focus:border-indigo-500 focus:ring-indigo-500 resize-none"></textarea>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 flex flex-wrap gap-3">
                ...
                <select v-model="priority" class="border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <select v-model="assignedTo" class="border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div class="space-y-3">
                <div v-for="task in tasks.data" :key="task.id"
                    :class="['rounded-lg border p-4 bg-white', statusColors[task.status] ?? 'bg-white border-gray-200']">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="flex flex-col items-end gap-1 shrink-0">
                            <span class="text-xs text-gray-500 capitalize">{{ task.status?.replace('_', ' ') }}</span>
                ...
                <p v-if="!tasks.data?.length" class="text-center text-gray-400 text-sm py-8">No tasks found.</p>
            </div>

            <div v-if="tasks.meta?.last_page > 1" class="flex items-center justify-between text-sm text-gray-600">
                <span>Showing {{ tasks.meta.from }}–{{ tasks.meta.to }} of {{ tasks.meta.total }}</span>
                <div class="flex gap-2">
                    <a v-if="tasks.links?.prev" :href="tasks.links.prev" class="px-3 py-1 border border-gray-300 rounded hover:bg-gray-50">Previous</a>
                    <a v-if="tasks.links?.next" :href="tasks.links.next" class="px-3 py-1 border border-gray-300 rounded hover:bg-gray-50">Next</a>
                </div>
            </div>
        </div>
    </AppLayout>
