...
const props = defineProps({ user: Object, roles: Array, tenants: Array });
...
function resetPassword() {
    passwordForm.put(`/admin/users/${props.user.id}/reset-password`, {
        preserveScroll: true,
            ...
            <form @submit.prevent="resetPassword" class="bg-white rounded-lg shadow-sm p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <TextInput v-model="passwordForm.password" type="password" class="mt-1 block w-full" autocomplete="new-password" />
                        <InputError :message="passwordForm.errors.password" class="mt-1" />
                    </div>
                    <div>
                        <InputLabel value="Confirm new password" :required="true" />
                        <TextInput v-model="passwordForm.password_confirmation" type="password" class="mt-1 block w-full" autocomplete="new-password" />
                    </div>
                </div>
                <div class="flex items-center justify-end gap-3">
                    <transition enter-active-class="transition" enter-from-class="opacity-0">
